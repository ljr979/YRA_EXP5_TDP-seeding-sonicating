...
def bar(fit_params, output_folder, monomer_only, group='t1', hue_var='t2', pal='BuGn', x_var='t3', y_var='tlag', savefig=False, ylabel='Lag Phase (h)', xlabel='Seed added (%)'):
    ...
    for i, (t1, df) in enumerate(fit_params.groupby(group)):
        ...
        sns.barplot(df1, x=x_var, y=y_var, hue=hue_var, palette=new_pal, dodge=True, edgecolor='darkgray', linewidth=0.5,capsize=0.1, ax=axes[i])
        axes[i].annotate(f'{t1}', xy=(-0.4, 14.8))
        sns.stripplot(data=df1, x=x_var, y=y_var, hue=hue_var, dodge=True, jitter=False, palette=new_pal, edgecolor='black', alpha=0.5, linewidth=1, ax=axes[i])
        axes[i].legend_.remove() # remove the second legend
...
if __name__ == "__main__":    
    ...
    fit_params = fit_params[fit_params['rquare']>0.93]

    #to plot multiple at the same time
    params_to_plot = ['tlag', 'slope', 'V50', 'top']
    p_dict = {
        'TDPLCD':'Purples',
        'TDPmRUBY':'Reds',
        'TDPt25':'Blues',
    ...
    for param in params_to_plot:
        ...
        bar(fit_params, output_folder, monomer_only, group='t1', hue_var='t2', pal=p_dict, x_var='t3', y_var=param, savefig=True, ylabel=param, xlabel='Seed added (%)')
    ...
    bar(fit_params, group='t1', hue_var='t2', pal='BuGn', x_var='t3', y_var='tlag', savefig=False, ylabel='Lag Phase (h)', xlabel='Seed added (%)')